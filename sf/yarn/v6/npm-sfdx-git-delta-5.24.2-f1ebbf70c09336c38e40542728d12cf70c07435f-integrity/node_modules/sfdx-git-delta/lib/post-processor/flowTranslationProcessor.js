...
const { FLOW_XML_NAME, META_REGEX, METAFILE_SUFFIX, TRANSLATION_EXTENSION, TRANSLATION_TYPE, } = require('../utils/metadataConstants');
const { writeFile, scanExtension, isSubDir, readFile, } = require('../utils/fsHelper');
const { pathExists } = require('fs-extra');
const { parse, join } = require('path');
const { buildIgnoreHelper } = require('../utils/ignoreHelper');
const { asArray, parseXmlFileToJson, xml2Json, convertJsonToXml, } = require('../utils/fxpHelper');
...
class FlowTranslationProcessor extends BaseProcessor {
    ...
    async _handleFlowTranslation() {
        for (const translationPath of this.translationPaths.keys()) {
            ...
            if (this.config.generateDelta) {
                ...
                this._scrapTranslationFile(jsonTranslation, this.translationPaths.get(translationPath));
    ...
    _scrapTranslationFile(jsonTranslation, actualFlowDefinition) {
        const flowDefinitions = asArray(jsonTranslation.Translations?.flowDefinitions);
        const fullNames = new Set(flowDefinitions.map(flowDef => flowDef.fullName));
        const strippedActualFlowDefinition = actualFlowDefinition.filter(flowDef => !fullNames.has(flowDef.fullName));
    ...
    async _getTranslationAsJSON(translationPath) {
        const translationPathInOutputFolder = join(this.config.output, treatPathSep(translationPath));
