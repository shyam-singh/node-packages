...
const { EOLRegex, getStreamContent, treatPathSep, } = require('./childProcessUtils');
const FOLDER = 'tree';
...
const readPathFromGitAsBuffer = async (path, { repo, to }) => {
    ...
    const bufferData = await getStreamContent(spawn('git', [...showCmd, `${to}:${normalizedPath}`], {
        cwd: repo,
    }));
    return bufferData;
};
...
module.exports.scanExtension = (dir, ext, config) => filterExt(scan(dir, config), ext);
