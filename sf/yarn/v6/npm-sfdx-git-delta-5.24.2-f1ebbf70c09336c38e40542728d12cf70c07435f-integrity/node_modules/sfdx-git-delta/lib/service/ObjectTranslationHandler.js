...
const { OBJECT_TRANSLATION_META_XML_SUFFIX, } = require('../utils/metadataConstants');
...
class ObjectTranslationHandler extends ResourceHandler {
    ...
    async _copyObjectTranslation(path) {
        ...
        const metadataDiff = new MetadataDiff(this.config, this.metadata, inFileMetadata);
    ...
    getObjectTranslationPath() {
        ...
        return `${parse(this.line).dir}${sep}${this.splittedLine[this.splittedLine.length - 2]}.${OBJECT_TRANSLATION_META_XML_SUFFIX}`;
    }
    _delegateFileCopy() {
