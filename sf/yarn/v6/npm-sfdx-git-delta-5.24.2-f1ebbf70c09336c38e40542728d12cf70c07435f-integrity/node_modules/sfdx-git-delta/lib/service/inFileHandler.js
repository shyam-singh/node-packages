'use strict';
const { LABEL_EXTENSION, LABEL_XML_NAME, } = require('../utils/metadataConstants');
const StandardHandler = require('./standardHandler');
const { basename } = require('path');
const { writeFile } = require('../utils/fsHelper');
const { getInFileAttributes, isPackable, } = require('../metadata/metadataManager');
...
const getNamePrefix = ({ subType, line }) => subType !== LABEL_XML_NAME ? `${getRootType(line)}.` : '';
class InFileHandler extends StandardHandler {
    ...
    _fillPackage(store, subType, member) {
        ...
        if (isPackable(subType)) {
            const cleanedMember = cleanUpPackageMember(`${getNamePrefix({ subType, line: this.line })}${member}`);
