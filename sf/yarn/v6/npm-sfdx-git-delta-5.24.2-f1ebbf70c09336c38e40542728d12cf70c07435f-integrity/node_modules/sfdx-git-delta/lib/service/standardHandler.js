...
const { ADDITION, DELETION, GIT_DIFF_TYPE_REGEX, MODIFICATION, } = require('../utils/gitConstants');
...
class StandardHandler {
    ...
    _parseLine() {
        return this.line.match(new RegExp(`(?<path>.*[/\\\\]${RegExpEscape(this.metadata.get(this.type).directoryName)})[/\\\\](?<name>[^/\\\\]*)+`, 'u'));
    }
    _getRelativeMetadataXmlFileName(path) {
        return `${parse(path).base.replace(this.ext, '').replace(/\.$/, '')}.${this.metadata.get(this.type).suffix}${METAFILE_SUFFIX}`;
    }
    _isProcessable() {
