...
function toValidatorFn(validator) {
    return (val) => {
        if (ts_types_1.isString(validator))
            return new RegExp(validator).test(val);
        if (ts_types_1.isInstance(validator, RegExp))
            return validator.test(val);
        if (ts_types_1.isFunction(validator))
            return !!validator(val);
        return true;
    };
}
function merge(kind, flag, describable) {
    if (ts_types_1.has(flag, 'validate') && ts_types_1.hasFunction(flag, 'parse')) {
...
function buildHelp(options) {
    ...
    return merge('help', command_1.flags.help(options), {
        description: ts_types_1.ensure(flag.description),
    });
}
...
function buildInteger(options) {
    ...
    return option(kind, options, (val) => {
        const parsed = kit_1.toNumber(val);
...
function buildMilliseconds(options) {
    ...
    return option(kind, options, (val) => {
        const parsed = kit_1.toNumber(val);
        validateValue(Number.isInteger(parsed), val, kind);
        return kit_1.Duration.milliseconds(validateBounds(kind, parsed, options, (v) => (ts_types_1.isNumber(v) ? v : v[kind])));
    });
}
function buildMinutes(options) {
    ...
    return option(kind, options, (val) => {
        const parsed = kit_1.toNumber(val);
        validateValue(Number.isInteger(parsed), val, kind);
        return kit_1.Duration.minutes(validateBounds(kind, parsed, options, (v) => (ts_types_1.isNumber(v) ? v : v[kind])));
    });
}
...
function buildSeconds(options) {
    ...
    return option(kind, options, (val) => {
        const parsed = kit_1.toNumber(val);
        validateValue(Number.isInteger(parsed), val, kind);
        return kit_1.Duration.seconds(validateBounds(kind, parsed, options, (v) => (ts_types_1.isNumber(v) ? v : v[kind])));
    });
}
...
function resolve(opts, key, def) {
    return ts_types_1.hasString(opts, key) ? opts[key] : def;
}
...
// tslint:disable-next-line:no-any matches oclif
    ...
    ts_types_1.definiteEntriesOf(flagsConfig).forEach(([key, flag]) => {
        if (isBuiltin(flag)) {
            if (!ts_types_1.isKeyOf(exports.optionalBuiltinFlags, key)) {
