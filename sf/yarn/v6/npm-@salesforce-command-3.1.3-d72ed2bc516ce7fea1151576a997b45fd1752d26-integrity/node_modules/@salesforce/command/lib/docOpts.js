...
class DocOpts {
    constructor(cmd) {
        ...
        this.flags = {};
        this.flagList = ts_types_2.definiteEntriesOf(this.cmd.flags)
            .filter(([, v]) => !v.hidden)
            .map(([k, v]) => {
            const { description, ...rest } = v;
            const flag = { description: ts_types_1.ensure(description), ...rest, name: k };
            this.flags[k] = flag;
            return flag;
        });
    }
    ...
    toString() {
        try {
            ...
            if (varargs) {
                ...
                const isRequired = ts_types_1.isPlainObject(varargs) && varargs.required;
                if (!isRequired) {
    ...
    groupFlagElements() {
        ...
        for (const flag of this.flagList) {
            if (ts_types_1.isArray(flag.dependsOn)) {
                this.combineElementsToFlag(elementMap, flag.name, flag.dependsOn, ' ');
            }
            if (ts_types_1.isArray(flag.exclusive)) {
        ...
        for (const remainingFlagName of Object.keys(this.flags)) {
            const remainingFlag = ts_types_1.ensure(this.flags[remainingFlagName]);
    ...
    combineElementsToFlag(elementMap, flagName, flagNames, unionString) {
        ...
        let isRequired = ts_types_1.ensure(this.flags[flagName]).required;
        if (!ts_types_1.isBoolean(isRequired) || !isRequired) {
    ...
    generateElements(elementMap = {}, flagGroups) {
        ...
        for (const flag of flagGroups) {
            const kind = ts_types_1.ensure(ts_types_1.getString(flag, 'kind'));
            // not all flags have short names
            const flagName = flag.char ? `-${flag.char}` : `--${flag.name}`;
            let type = '';
            if (kind !== 'boolean') {
                if (kind === 'enum') {
                    const options = ts_types_1.ensureArray(ts_types_3.get(flag, 'options'));
                    type = ` ${options.join('|')}`;
                }
                else {
                    type = ` <${kind || 'string'}>`;
