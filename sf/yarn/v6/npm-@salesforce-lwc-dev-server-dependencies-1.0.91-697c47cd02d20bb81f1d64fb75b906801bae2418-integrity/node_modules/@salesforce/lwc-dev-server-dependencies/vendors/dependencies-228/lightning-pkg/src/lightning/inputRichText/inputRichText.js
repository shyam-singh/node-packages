...
const INPUTABLE_NODES = /^input$|^select$|^textarea$|^button$/;
...
const DEFAULT_FONT_NAME_VALUE = 'default',
...
function throwFriendlyError(e) {
    if (e) {
        e.message = e.message
            ? '[inputRichText_lwc]' + e.message
            : 'An error occurred in inputRichText_lwc.';
        throw new Error(e);
    }
}
/**
 * A WYSIWYG editor with a customizable toolbar for entering rich text
 * @slot toolbar Placeholder for lightning-rich-text-toolbar-button-group
 */
export default class LightningInputRichText extends LightningElement {
    /*
        Public attributes:
            ...
            formats             {List}    - A list of allowed formats. By default, the list is computed based on enabled categories.
    ...
    */
    ...
    quill; // Quill api
    initialRender = true; // Guard for initializing during first render cycle
     ...
     *
     * This includes the standard buttons as well as
     * the custom buttons that are passed in
     *
    ...
    setupButtons() {
        ...
        ).concat(findAllFocusableNodes(this.template.querySelector('slot')));
    ...
    addTagMatchers() {
        ...
        // This matcher detects tables. If tables are disabled in custom formats,
        // it will return a normal delta, otherwise it will apply the table format
        this.quill.clipboard.addMatcher('table', node => {
            if (this.formats && this.formats.indexOf('table') === -1) {
    ...
    ******************************** */
     ...
     *      1. Determine the current button with focus by capturing event's target
    ...
    navigateToolbar(evt) {
        ...
        // If element with focus is a custom element (custom button),
        // get actual element inside shadow root with focus
        if (!(target && target.classList.contains('slds-button'))) {
            target = getElementWithFocus(target);
        }

        // Retrieve non-disabled standard and custom buttons present in the toolbar
        // Retrieve each time in case there are changes in the toolbar buttons
        // like (new) disabled buttons or different custom buttons
        const standardButtonList = Array.from(
            this.template.querySelectorAll(TOOLBAR_SELECTOR)
        ).filter(button => !button.disabled);
        const customButtonList = findAllFocusableNodes(
            this.template.querySelector('slot[name="toolbar"]')
        ).filter(node => !node.disabled);

        const buttonList = standardButtonList.concat(customButtonList);

        // If the element with focus is an inputable node,
        // move left or right based on user input
        if (
            target &&
            INPUTABLE_NODES.test(target.tagName.toLowerCase()) &&
            !target.disabled
        ) {
    ...
    imageButtonClickHandler() {
        const input = this.template.querySelector('.image-picker');
        if (input) {
            // Clear previous input value so change event will be fired on select
            input.value = '';
            input.click();
        }
    }
    ...
    /**
     * Insert the selected image to the editor
     */
    handleInsertImage(evt) {
    ...
    openLinkPanel(value) {
        ...
        createLinkPanel.show(linkButton, {
            reference: {
                vertical: this.isBottomToolbar ? 'top' : 'bottom',
                horizontal: 'center',
            },
            popup: {
                vertical: this.isBottomToolbar ? 'bottom' : 'top',
                horizontal: 'center',
            },
            padding: 0.15,
        });
    ...
    closeLinkPanel() {
        this.template.querySelector('[data-link-popup]').close();
    }
    ...
    selectFont(fontChangeEvt) {
        ...
        const selectFont = fontChangeEvt.target.value;
    ...
    selectSize(sizeChangeEvt) {
        ...
        const selectSize = sizeChangeEvt.target.value;
        const quillApi = this.quill;
        quillApi.focus();
    ...
    /**
     * Replaces a range of text in the rich text editor with a new string
     *
     ...
     * @param {String} selectMode - Defines how the selection is set after the text is replaced. Valid values are select, start, end, and preserve.
     */
    @api
    setRangeText(replacement, start, end, selectMode) {
        ...
        try {
            inputRichTextLibrary.setRangeText(
                this.quill,
                replacement,
                start,
                end,
