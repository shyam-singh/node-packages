Webruntime.define('@webruntime/app', ['@app/basePath', 'webruntime_loader/loader', '@app/csrfToken', '@app/routes', '@salesforce/user/isGuest', '@app/viewToThemeLayoutMap', '@app/brandingProperties'], function (basePath, loader, csrfToken, routes, isGuest, viewToThemeLayoutMap, props) { 'use strict';
    ...
    function sendError({
      ...
      const url = basePath + ERRORS_PATH_PREFIX;
      const {
        pathname,
        hostname
      } = window.location;
    ...
    */
    ...
    function transferProperty(obj = {}, to, from, fromProp, defaultValue, toProp = fromProp) {
      ...
      obj = _objectSpread({}, obj, {
        [to]: _objectSpread({}, obj[to], {
          [toProp]: isObject$3(obj[from]) && obj[from][fromProp] || defaultValue
        }),
        [from]: _objectSpread({}, obj[from])
      });
    ...
    async function fetch(path, init = {}) {
      ...
      const initParams = _objectSpread$1({}, init, {
        headers: _objectSpread$1({}, init.headers),
    ...
    function sendBeacon({
      ...
      const sentBeacon = window.navigator && window.navigator.sendBeacon && window.navigator.sendBeacon(url, payload);
    ...
    const supports = {
      ...
      pathname: Boolean(window.location && window.location.pathname !== undefined && window.location.pathname !== null),
      rtt: Boolean(window.navigator && window.navigator.connection && window.navigator.connection.rtt),
    ...
    function prepPayload(payload) {
      const stringifiedPayload = JSON.stringify(payload); // trims the size of the payload by rounding all numbers to max 2 decimal places
    ...
    if (supports.PerformanceObserver && supports.PerformancePaintTiming) {
      ...
      observer.observe({
        entryTypes: [PERFORMANCE_ENTRY_TYPE.PAINT]
      });
    } else {
    ...
    function performanceMarkHelper(ns, name, ctx) {
      const markName = getMarkName$1(ns, name);
      window.performance.mark(markName);
    }

    function mark(ns, name, ctx) {
      performanceMarkHelper(ns, name);
    }
    ...
    function markEnd(ns, name, ctx) {
      ...
      try {
        ...
        logError$1(`[instrumentation] no startMark named ${markName} found`, ex.stack);
      }
    }
    ...
    const labelSecondsAgo = 'a few seconds ago'; // These labels will only be used as fallback in browsers that do not support Intl.RelativeTimeFormat
    ...
    function register$1(adapterId, adapterFactory) {
      {
        assert$2.isTrue(adapterId, 'adapter id must be truthy');
        assert$2.isTrue(typeof adapterFactory === 'function', 'adapter factory must be a callable');
      }
    ...
    function isSimpleRoute(route) {
      ...
      return isValidRoute(route) && isObject$4(route.attributes) && Object.keys(route.attributes).length === 1 && Object.prototype.hasOwnProperty.call(route.attributes, 'path');
    }
    /**
     * f(route) -> true/false
     *
    ...
    function isValidRoute(route) {
      ...
      return isObject$4(route) && (Object.prototype.hasOwnProperty.call(route, 'id') || Object.prototype.hasOwnProperty.call(route, 'type'));
    }
    ...
    */
    ...
    function getAttributesFromPathAndRouteDef(path, routeDef) {
      ...
      if (path && routeDef) {
        ...
        if (routeDef.original.page && isObject$4(routeDef.original.page.attributes)) {
    ...
    function getQueryStringFromRoute(route = {}) {
      ...
      return Object.prototype.hasOwnProperty.call(route, 'state') && isObject$4(route.state) ? getQueryString(route.state) : '';
    }
    /**
     * f(URL, routes[]?) -> route
    ...
    const NAV_CONTEXT_EVENT = `universalcontainercontextevent${guid()}`; // Keep a cache of context metadata, with their IDs as keys:
    //    id => {
    //              id,
    ...
    function getNavigationContextId(node = window, isWired) {
      ...
      node.dispatchEvent(isWired ? new LinkContextEvent(NAV_CONTEXT_EVENT, callback) : new CustomEvent(NAV_CONTEXT_EVENT, {
        bubbles: true,
        composed: true,
        detail: callback
      }));
    ...
    function getNavigationContext(id) {
      ...
      invariant$2(metadata && metadata.value, messages.MISSING_CONTEXT);
    ...
    function validateContext(context) {
      invariant$2(context === undefined || isObject$4(context) && typeof context.navigate === 'function' && typeof context.generateUrl === 'function' && typeof context.subscribe === 'function' && Object.keys(context).length === 3, messages.INVALID_CONTEXT);
      return context;
    }
    /**
    ...
    register$1(NavigationContext, target => {
    ...
    const NAV_EVENT = `universalcontainernavigationevent${guid()}`; // Internal event fired to find nearest ancestor.
    ...
    class NavNode {
      ...
      addChild(child) {
        ...
        setTimeout(() => {
          invariant$2(!this.child, messages.MULTIPLE_CHILDREN);
          this.child = child;
        }, 0);
      }
      /**
    ...
    class Router extends NavNode$1 {
      ...
      constructor(c = {}, node = window) {
        ...
        this.preFilters = createFilterChain();
        this.postFilters = createFilterChain();
        this.errorFilters = createFilterChain();
      ...
      parseRouteDefinitions(routeDefs) {
        this.routes = routeDefs.map(def => {
          invariant$2(def.path, messages.MISSING_PATH);
          const params = [];
      ...
      defaultGetRouteFromUrl(url) {
        ...
        const info = getRouteFromUrl(hasPrefix ? url.replace(this.prefix, '') : url, hasPrefix ? this.routes : []); // If the prefix was stripped off of a Simple Route, add it back on, so the path is absolute.
      ...
      async process(url, options) {
        const info = freeze$2(this.getRouteFromUrl(url));
        const route = freeze$2(info.route); // Run the root -> leaf chain of pre navigate filters, if this is the root.
        ...
        this.infoMatch = info;
        this.currentUrl = url;
        this.routeObservable.next(route, freeze$2(info.data)); // Delegate to a child.
      ...
      preProcess(url, info = freeze$2(this.getRouteFromUrl(url))) {
        const route = freeze$2(info.route); // Check that the URL has a matching route, otherwise it is an error.
        ...
        this.pathMatch = !isSimpleRoute(route) ? getPathFromUrl(this.getUrlFromRoute(route).url || '') : this.parent ? this.parent.pathMatch : ''; // Compile this router's filters; continue with TRUE if there are no filters.
        ...
        .then(canContinue => {
          return canContinue ? canContinue : generateMessageObject(messages.PRENAV_FAILED, [url]);
        });
      }
      /**
      ...
      navigate(input, options) {
        ...
        if (url === null) {
          this.root.processError(generateMessageObject(messages.MISSING_URL, [input.id || input.type]));
          return;
        } // Fire event up the DOM with the original caller input


        this.node.dispatchEvent(new CustomEvent(NavNode$1.navigationEvent, {
          bubbles: true,
          composed: true,
          detail: {
            url,
    ...
    class HistoryRouter extends Router$1 {
      /**
       * Override.
    ...
    function createNavigationContext(node, config = {}) {
      ...
      const routerAPI = {
        ...
        connect: () => {
          ...
          invariant$2(!hasRoot || !!newRouter.parent, messages.MULTIPLE_ROOTS);
    ...
    const router = createRouter(config).addPreNavigate(isAuthorized).addErrorNavigate(handleError);
    router.connect();
    /**
    ...
    function getRouteFromUrl$1(url, defaultImpl) {
      let {
        route,
        data
      } = defaultImpl(url); // Return the default route if no match is found for the URL
    ...
    function getUrlFromRoute$1(route = {}, defaultImpl) {
      const {
        type,
        attributes: {
          url
        } = {}
      } = route; // Return URL directly from standard__webPage types
    ...
    async function isAuthorized({
      ...
      if (isGuest && 'isPublic' in data && !data.isPublic) {
        const url = await generateUrl$1(route);
    ...
    function navigate$1(loc, options) {
      navigate(router.id, loc, options);
    }
    /**
     * Generate a URL for the given route.
     *
    ...
    function generateUrl$1(route) {
      return generateUrl(router.id, route);
    }
    /**
     * Subscribe to navigation state changes.
     *
    ...
    function subscribe$1(callback, replay = true) {
      subscribe(router.id, callback, replay);
    ...
    async function handleUiApiCall({
      ...
      const remainingParams = params && _objectSpread$4({}, params) || {}; // replace the path params
    ...
    class App extends BaseLightningElement {
      constructor(...args) {
        super(...args);
        this.generatedTemplateHtml = _tmpl$1;
        this.attributes = {};
        this.routeParams = {};
       ...
       */
      connectedCallback() {
        this.subscription = subscribe$1(this.setRoute.bind(this));
      }
      ...
      async setRoute({
        ...
        const params = _objectSpread$6({}, state, attributes);
        ...
        return loader.load(getViewModuleSpecifier(themeLayout)).then(module => module.default).then(tmpl => {
    ...
    register$1(CurrentPageReference, target => {
      ...
      target.addEventListener('connect', () => {
        if (!subscription) {
          subscription = subscribe(getNavigationContextId(target, true), pageRef => {
            ...
            const pageRefCopy = _objectSpread$7({}, pageRef);
    ...
    const NavigationMixin = Base => {
      invariant$2(typeof Base.prototype.dispatchEvent === 'function', messages.INVALID_MIXIN_CMP, [Base]);
    ...
    class Link extends BaseLightningElement {
      constructor(...args) {
        super(...args);
        this.path = void 0;
        this.route = void 0;
    ...
    class RouterContainer extends BaseLightningElement {
      ...
      async setRoute({
        ...
        const viewModuleSpecifier = getViewModuleSpecifier(view); // load the new module and report any errors
        ...
        try {
          ...
          this.generatedTemplateHtml = _tmpl$3;
          module = await loader.load(viewModuleSpecifier);
        } catch (error) {
          reportError$1({
        ...
        if (module) {
          ...
          this.generatedTemplateHtml = template.html; // set the route params for the template

          this.routeParams = _objectSpread$8({}, state, attributes);
...

    {
      ...
      Object.entries(props).forEach(([key, value]) => document.documentElement.style.setProperty(key, value));
    }
    ...
    customElements.define('webruntime-app', buildCustomElementConstructor(App$1));
    window.performance.measure(`${WEBRUNTIME_PREFIX}-framework-bootstrap`);
