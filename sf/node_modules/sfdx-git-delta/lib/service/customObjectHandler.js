...
const { FIELD_DIRECTORY_NAME, MASTER_DETAIL_TAG, OBJECT_TYPE, } = require('../utils/metadataConstants');
...
class CustomObjectHandler extends StandardHandler {
    ...
    async _handleMasterDetailException() {
        ...
        const masterDetailsFields = await asyncFilter(fields, async (fieldPath) => {
            const content = await readPathFromGit(join(fieldsFolder, fieldPath), this.config);
            return content.includes(MASTER_DETAIL_TAG);
        });
        await Promise.all(masterDetailsFields.map(field => this._copyWithMetaFile(join(fieldsFolder, field))));
    }
}
module.exports = CustomObjectHandler;
//# sourceMappingURL=customObjectHandler.js.map