...
const addToStore = store => ({ type, member }) => {
    fillPackageWithParameter({ store, type, member });
    return store;
};
const hasMember = store => attributes => subType => member => store.get(attributes.get(subType)?.xmlName)?.has(member);
...
const getSubTypeTags = attributes => fileContent => Object.keys(getRootMetadata(fileContent)).filter(tag => attributes.has(tag));
const extractMetadataForSubtype = fileContent => subType => asArray(getRootMetadata(fileContent)?.[subType]);
// Diff processing functional area
const compareContent = attributes => (contentAtRef, otherContent, predicat) => getSubTypeTags(attributes)(contentAtRef)
    .flatMap(processMetadataForSubType(contentAtRef, otherContent, predicat, attributes))
    .reduce((store, nameByType) => addToStore(store)(nameByType), new Map());
const processMetadataForSubType = (baseContent, otherContent, predicat, attributes) => subType => {
    ...
    const processElement = getElementProcessor(subType, predicat, otherMeta, attributes);
...
const generatePartialJSON = attributes => jsonContent => store => {
    ...
    return getSubTypeTags(attributes)(jsonContent).reduce((acc, subType) => {
        ...
        rootMetadata[subType] = meta.filter(elem => storeHasMemberForType(key(elem)));
        return acc;
    }, jsonContent);
};
class MetadataDiff {
    ...
    prune(jsonContent = this.toContent, elements = this.add) {
        const prunedContent = generatePartialJSON(this.attributes)(jsonContent)(elements);
