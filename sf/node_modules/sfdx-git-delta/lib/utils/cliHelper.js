...
const { getLatestSupportedVersion, isVersionSupported, } = require('../metadata/metadataManager');
...
class CLIHelper {
    ...
    _filterDirectories() {
        return asyncFilter([this.config.output, join(this.config.repo, this.config.source)].filter(Boolean), async (dir) => {
    ...
    async _getApiVersion() {
        const isInputVersionSupported = await isVersionSupported(this.config.apiVersion);
        ...
        if (!isInputVersionSupported) {
            ...
            if (this.config.apiVersion !== undefined &&
                ...
                this.work.warnings.push({
                    message: format(messages.warningApiVersionNotSupported, latestAPIVersionSupported),
                });
            }
...
CLIHelper.TO_DEFAULT_VALUE = 'HEAD';
CLIHelper.OUTPUT_DEFAULT_VALUE = './output';
CLIHelper.SOURCE_DEFAULT_VALUE = './';
CLIHelper.REPO_DEFAULT_VALUE = './';
