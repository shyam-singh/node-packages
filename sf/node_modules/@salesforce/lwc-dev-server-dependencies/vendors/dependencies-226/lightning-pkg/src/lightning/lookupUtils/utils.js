...
/**
 * Updates referenceInfos based on whether it has createNew enabled
 * @param {Array} actions - An array of lookup actions received from @wire(getLookupActions)
 * @param {Object} referenceInfos - The reference api infos.
 */
function mapActionsToReferenceInfos(actions, referenceInfos) {
    for (const apiName in actions) {
        if (
            Object.prototype.hasOwnProperty.call(actions, apiName) &&
            Object.prototype.hasOwnProperty.call(referenceInfos, apiName)
        ) {
            referenceInfos[apiName].createNewEnabled = hasCreateFromLookup(
                actions[apiName].actions
            );
        }
    }
}
...
function mapLookupWireRecords(records, nameField, iconDetails, optionType) {
    return records.map(record => {
        ...
        return {
            ...
            type: optionType,
            value:
                (fields.ExternalId && fields.ExternalId.value) ||
                fields.Id.value,
            externalObjectValue:
                fields.ExternalId && fields.ExternalId.value
                    ? fields.Id.value
...
/**
 * Creates a map of normalized record ids.
 * @param {Array} value - An array of record ids.
 */
function mapNormalizedRecordIds(value) {
    return value
...
function mapRecordUiWireRecords(
    ...
    const pills = recordsWithReferencedApiNameField
        ...
        .map(record => ({
            ...
            type: COMMON_LOOKUP_CONSTANTS.PILL_TYPE_ICON,
            value:
                (record.fields.ExternalId && record.fields.ExternalId.value) ||
                record.id,
            externalObjectValue:
                record.fields.ExternalId && record.fields.ExternalId.value
                    ? record.id
...
function parseLdsError(error) {
    return { message: parseError(error).message };
}
...
export {
    ...
    mapActionsToReferenceInfos,
    mapLookupWireRecords,
    mapNormalizedRecordIds,
